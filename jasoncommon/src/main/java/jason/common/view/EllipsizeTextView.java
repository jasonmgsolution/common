...
/**
 * Created by jasonmg_0302 on 2017-03-14.
 */
...
public class EllipsizeTextView extends AppCompatTextView {
    ...
    private void init(){
        Spannable word = new SpannableString("  ...더보기");
        word.setSpan(new ForegroundColorSpan(ContextCompat.getColor(getContext(), R.color.gray_7)), word.length() -word.length() , word.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        setEllipsizeText(word, 0);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (layout != null && originalText.split(System.getProperty("line.separator")).length > mMaxLines -1) {
            if (!getText().toString().contains(mEllipsizeText.toString())) {
    ...
    public void setText(CharSequence text, BufferType type) {
        ...
        super.setText(originalText.trim(), type);
        if (mIsExactlyMode) {
            requestLayout();
        }
    }
